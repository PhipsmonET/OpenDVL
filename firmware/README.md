...
Acquisizione Periferiche (Hardware Layer):
  TIM2 (Timer Trigger 10-20 MHz) -> Richiesta Hardware DMA
  GPIOD (12-bit Ch A) & GPIOE (12-bit Ch B) -> Lettura parallela IDR via DMA -> Buffer in RAM D1

Moduli Applicativi (Software Tasks):
  TX Task: Genera burst 1 MHz su TIM1 (PE9, PE11) e avvia la rampa DAC (PA4, PA5) per il guadagno AD600 (TVG)
  RX Task: Demultiplexa e decima i dati del buffer circolare DMA in RAM D1
  DSP Task: Applica demodulazione IQ, filtri FIR (CMSIS-DSP) e calcola l'autocorrelazione Doppler
  Navigation Task: Esegue la calibrazione Clay-Medwin e converte i vettori in Vx/Vz (Matrice Janus 2D)
  Telemetry Task: Confeziona le stringhe seriali NMEA e le trasmette via UART (RS485) o Ethernet
...
Dove:
- T è la temperatura misurata (in gradi Celsius).
